...
from PyQt5.QtWidgets import QDialog, QMainWindow, QMessageBox  # QComboBox,
...
from lbk_library.testing_support import datafile_close


def base_setup(qtbot):
    datafile = DataFile()
    main = QMainWindow()
...
def test_06_01_class_type(qtbot):
    dialog, main, datafile = base_setup(qtbot)
    assert isinstance(dialog, Dialog)
    assert isinstance(dialog, QDialog)
    datafile_close(datafile)


def test_06_02_get_datafile(qtbot):
    dialog, main, datafile = base_setup(qtbot)
...
def test_06_06_msg_quest_change(qtbot):
    dialog, main, datafile = base_setup(qtbot)
    changed_name = "description"
...
def test_06_08_msg_warning_selectio(qtbot):
    dialog, main, datafile = base_setup(qtbot)
    name = "Source"
    action = "Save"
    msg = "Nothing to "
...
def test_06_12_set_combo_box_selections(qtbot):
    dialog, main, datafile = base_setup(qtbot)
    dialog.form = A_Form(dialog)
    ids = ["1", "3", "7", "10", "15", "60"]
    entry = "7"
